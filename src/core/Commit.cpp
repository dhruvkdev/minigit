...
std::string Commit::getCommitId() const {
    return commitId;
}

std::string Commit::getCommitMsg() const {
    return commitMsg;
}

std::string Commit::getParentCommitId() const {
...
std::unordered_map<std::string, std::string> Commit::getFileBlob() const {
    return fileBlob;
}
...
Commit Commit::deserialize(const std::string& filename) {
    ...
    if (!in.is_open()) {
        throw std::runtime_error("Failed to open file for reading"); // TODO: Add try-catch handling
    }
